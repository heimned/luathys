...
try:
    from timeline_log import TimelineLog
except ImportError:
    TimelineLog = None
...
SIGNATURES = {"luau": [b"\x03\x30\x00\x00", b"\x19\x00\x00\x00"]}

# SUNC docs: https://docs.sunc.sh/
class MemoryScanner:
...
class LuathysApp(QMainWindow):
    ...
    def apply_style(self):
        ...
        QApplication.instance().setStyleSheet("""
            ...
            QPushButton {
                background-color: #89b4fa; border: none; padding: 8px 16px;
            ...
            QProgressBar {
                ...
                background-color: #303446; height: 20px;
            }
    ...
    def build_ui(self):
        ...
        main_layout.addWidget(prf)

        # Timeline Log Component (replaces old text log)
        log_section = QFrame()
        log_layout = QVBoxLayout(log_section)
        log_title = QLabel("Activity Log")
        log_title.setStyleSheet("color: #89b4fa; font-weight: bold;")
    ...
    def check_process(self):
        self.log("Checking for Roblox process...", "info")
        ...
        if ok:
            ...
            self.log(f"Roblox found, PID: {res}", "success")
        else:
            self.proc_status.setText(f"Not found ({res})")
            self.indicator.setStyleSheet("color: #f38ba8;")
            self.log(f"Roblox not found: {res}", "error")
    ...
    def _scan_worker(self):
        self.log("Starting memory scan...", "info")
        def on_found(c):
            if not self.scanning: return
            self.found_label.setText(f"Scripts found: {c}")
        scripts = self.scanner.scan_memory(on_found)
        self.log(f"Found {len(scripts)} potential scripts", "success" if len(scripts) > 0 else "warning")
        svcs = self.scanner.scan_services()
        self.log(f"Found {len(svcs)} service markers", "info")
        if self.scanning:
            ...
            if len(scripts) > 0:
                self.log(f"Scan complete: {len(scripts)} scripts extracted", "success")
            else:
                self.log("Scan complete. Try joining a different game.", "warning")
        self.scanning = False
        self.scan_btn.setEnabled(True)
        self.stop_btn.setEnabled(False)
    ...
    def export(self):
        if not self.results:
            ...
            return
        self.log(f"Exporting {len(self.results)} scripts...", "info")
        ...
        for i, s in enumerate(self.results):
            try:
                ...
                self.log(f"Error {i}: {e}", "error")
        self.log(f"Export complete: {ok}/{len(self.results)} saved", "success")
        QMessageBox.information(self, "Done", f"Exported {ok} to {self.output_dir}")
...
def main():
    ...
    w = LuathysApp()
    w.show()
    w.log("Luathys Extractor ready", "info")
    w.log("Launch Roblox, then click 'Check Process'", "info")
    sys.exit(app.exec())
